...
use crew_llm::{LlmProvider, ProviderChain, RetryProvider, SwappableProvider};
use eyre::Result;
...
impl SwitchModelTool {
    fn handle_list(&self) -> Result<ToolResult> {
        ...
        for entry in crew_llm::registry::all_entries() {
    ...
    async fn handle_switch(&self, input: Input) -> ToolResult {
        ...
        let provider_name = match input
            .provider
            .or_else(|| crew_llm::registry::detect_provider(&model_name).map(String::from))
        {
            Some(name) => name,
            None => {
                return ToolResult {
        ...
        let entry = match crew_llm::registry::lookup(&provider_name) {
            Some(e) => e,
            None => {
                return ToolResult {
                    output: format!(
