...
impl Agent {
    ...
    async fn consume_stream_inner(
        ...
        // - TTFT (first token): generous — models need time to process large inputs before
        //   generating. Scales with input: base 30s + 1s per 1K input tokens, capped at 180s.
        // - Inter-chunk: once streaming starts, chunks arrive every <1s. If no chunk for 30s after
        //   first token, the stream is stalled.
