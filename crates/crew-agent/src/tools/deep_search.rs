...
//!
//! Saves each crawled page as a markdown file under `.crew/research/<query-slug>/`.
...
pub struct DeepSearchTool {
    ...
    client: Client,
    /// Research output base directory (e.g. ~/.crew/research/ or a sub-agent's research_dir).
...
impl Tool for DeepSearchTool {
    ...
    fn description(&self) -> &str {
        "Search the web and crawl all result URLs in parallel. Saves each page as a markdown file under .crew/research/<query>/. Returns an index of saved files — use read_file to examine specific pages for synthesis."
    }
