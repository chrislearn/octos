//! Local Cokret crypto state for octos channel adapters.
//!
//! The SDK owns the protocol objects (`CryptoStoreBinding`, `MemoryCryptoStore`
//! and `CokretMlsGroup`). This module gives octos a small file-backed wrapper
//! so account-mode and applet-mode can persist decryption failures, MLS group
//! snapshots, recovery plans and realm encryption policy under the gateway data
//! directory.
...
const STATE_VERSION: &str = "octos.cokret.crypto_state.v1";
...
impl CokretCryptoStateFile {
    fn new(scope_id: String) -> Result<Self> {
        ...
        Ok(Self {
            ...
            mls_store_json: store
                .export_backup_json()
                .map_err(|err| eyre!("cokret crypto store export: {err}"))?,
    ...
    fn mls_store(&self) -> Result<MemoryCryptoStore> {
        let mut store = MemoryCryptoStore::new();
        store
            .import_backup_json(&self.mls_store_json)
            .map_err(|err| eyre!("cokret crypto store import: {err}"))?;
        Ok(store)
    }

    fn set_mls_store(&mut self, store: &MemoryCryptoStore) -> Result<()> {
...
impl FileCokretCryptoStore {
    #[must_use]
    pub fn for_account(octos_home: &Path, channel_id: &str, account_id: &str) -> Self {
        let scope_id = account_scope_id(channel_id, account_id);
        Self::new(octos_home, scope_id)
    }

    #[must_use]
    pub fn for_applet(octos_home: &Path, config_id: &str) -> Self {
        let scope_id = applet_scope_id(config_id);
        Self::new(octos_home, scope_id)
    }

    #[must_use]
    pub fn new(octos_home: &Path, scope_id: String) -> Self {
        let path = octos_home
            .join("gateway")
            .join("cokret-crypto")
    ...
    pub fn feature_report() -> Result<FeatureSafetyReport> {
        let report = current_feature_safety_report();
        report
            .validate()
            .map_err(|err| eyre!("cokret crypto feature set is unsafe: {err}"))?;
        Ok(report)
    }

    pub fn load(&self) -> Result<CokretCryptoStateFile> {
        match std::fs::read(&self.path) {
            ...
            Ok(bytes) => {
                let mut state: CokretCryptoStateFile = serde_json::from_slice(&bytes)
                    .wrap_err_with(|| format!("parse {}", self.path.display()))?;
    ...
    pub fn save(&self, state: &CokretCryptoStateFile) -> Result<()> {
        if state.scope_id != self.scope_id {
            bail!(
        ...
        std::fs::write(&tmp, bytes).wrap_err_with(|| format!("write {}", tmp.display()))?;
    ...
    pub fn upsert_realm_policy(&self, policy: CokretRealmCryptoPolicy) -> Result<()> {
        let mut state = self.load()?;
        state.realm_policies.insert(policy.realm_id.clone(), policy);
        self.save(&state)
    }

    pub fn update_realm_policies_from_sync(&self, realms_value: &Value) -> Result<usize> {
    ...
    ) -> Result<CokretBootstrapRecord> {
        let mut state = self.load()?;
        let store = state.mls_store()?;
        let principal = Did::new(principal_id.to_owned())
            .wrap_err_with(|| format!("invalid Cokret principal DID '{principal_id}'"))?;
        let device = DeviceId::new(device_id.to_owned())
            .wrap_err_with(|| format!("invalid Cokret device id '{device_id}'"))?;
    ...
    pub fn record_unable_to_decrypt(
        ...
        let record = UnableToDecryptRecord {
            event_id: EventId::new(event_id.to_owned())
                .wrap_err_with(|| format!("invalid Cokret event id '{event_id}'"))?,
            realm_id: RealmId::new(realm_id.to_owned())
                .wrap_err_with(|| format!("invalid Cokret realm id '{realm_id}'"))?,
            sender: Did::new(sender.to_owned())
                .wrap_err_with(|| format!("invalid Cokret sender DID '{sender}'"))?,
            reason,
            encrypted_content,
    ...
    pub fn try_decrypt_content_block(
        ...
        let content = serde_json::from_slice(&plaintext)
            .wrap_err("decrypted Cokret content block is not JSON")?;
        let updated = group
            .persist_state(&mut store)
            .map_err(|err| eyre!("persist Cokret MLS group: {err}"))?;
    ...
    pub fn encrypt_content_block_for_realm(
        ...
        let payload = group
            ...
            .map_err(|err| eyre!("encrypt Cokret MLS payload: {err}"))?;
        group
            .persist_state(&mut store)
            .map_err(|err| eyre!("persist Cokret MLS group: {err}"))?;
        state.set_mls_store(&store)?;
